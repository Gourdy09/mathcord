...
function parseMatrix(matrixStr) {
  try {
    ...
    // Check if the matrix is square (required for determinant)
    if (size[0] !== size[1]) {
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const determinantEmbed = new EmbedBuilder()
        .setColor("#DC143C")
        .setTitle("Matrix Determinant")
        .setDescription("Calculating the determinant of the given matrix")
        .addFields(
          {
            name: "Input Matrix",
